...
export function computeDueDates(startDate: Date, count: number, frequency: Frequency): Date[] {
  ...
  const dates = Array.from({ length: count }, (_, i) => {
    if (frequency === 'DAILY')       return new Date(Date.UTC(y, m, d + i        + 2, 2, 55, 0))
    if (frequency === 'WEEKLY')      return new Date(Date.UTC(y, m, d + i * 7    + 2, 2, 55, 0))
    if (frequency === 'FORTNIGHTLY') return new Date(Date.UTC(y, m, d + i * 15   + 2, 2, 55, 0))
    /* MONTHLY */                    return new Date(Date.UTC(y, m + i, d        + 2, 2, 55, 0))
  })
