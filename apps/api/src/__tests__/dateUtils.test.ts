...
describe('computeDueDates', () => {
  ...
  test('WEEKLY generates the stored due-date sequence', () => {
    ...
    expect(dates).toHaveLength(3)
    expect(dates[0]).toEqual(storedDueDate(2025, 0, 8))
    expect(dates[1]).toEqual(storedDueDate(2025, 0, 15))
    expect(dates[2]).toEqual(storedDueDate(2025, 0, 22))
  })
  ...
  test('MONTHLY generates the stored due-date sequence', () => {
    ...
    expect(dates).toHaveLength(3)
    expect(dates[0]).toEqual(storedDueDate(2025, 1, 1))
    expect(dates[1]).toEqual(storedDueDate(2025, 2, 1))
    expect(dates[2]).toEqual(storedDueDate(2025, 3, 1))
  })
  ...
  test('first due date is the next local day, not the start day itself', () => {
